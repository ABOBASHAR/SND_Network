<?php
function validate_customer_data(&$fname,&$lname,&$email,&$package,&$password,&$Joining_date,&$errors){
    $errors=["fname_error" => "", "lname_error" => "", "email_error" => "", "package_error" => "", "password_error" => "","Joining_date_error" => ""];
...
#Last name validation
    ...
    $errors["lname_error"]="Error ... last name field required";
    }
    else{
    $lname=ucfirst(trim($_POST["lname"]));
//Ensure using the alphabetic character
    if(!preg_match("/[a-z]/", $lname)){
        $errors["lname_error"]="Invalid last name format.";
    }
}
#Email validation
...
#Date validation
        if (empty(trim($_POST["Joining_date"]))) {
            $errors["Joining_date_error"] = "Error ... Date field required.";
        } else {
